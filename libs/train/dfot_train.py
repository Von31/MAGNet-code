...
from libs.dataloaders import DataType, DatasetName, Hdf5Dataset, InterleavedDataset, collate_dataclass_list
from libs.model.dfot import config, network
...
def get_wandb_logger(configs):
    config_optimizer={
      ...
      "subseq_len": configs.model_cfg.sequence_length,
...
class TrainingConfig:
    ...
    vqvae_model_path: Path = Path('./experiments_vqvae/test/v0/best_checkpoints')
    ...
    random_variable_len_proportion: float = 1.0
    slice_method: str = "random_uniform_len"
    ...
    def _default_model_cfg():
        return config.DFOTConfig(
            ...
            loss_weight={
            "pose_latent": 1.0,
            "canonical_tm1_t_transforms": 2.0,
            "canonical_self_partner_transforms": 1.0,
            "body_contacts": 0.1,
            },
            vqvae_model_path=TrainingConfig.vqvae_model_path,
            vqvae_cfg=MISSING,
        )
...
def load_cfg(yaml_path: str = None):
    ...
    vqvae_train_cfg = OmegaConf.load(cfg['vqvae_model_path'] / '../config.yaml')
    vqvae_model_cfg = vqvae_train_cfg['model_cfg']
    cfg = OmegaConf.merge(cfg, {'model_cfg': {'vqvae_cfg': vqvae_model_cfg}})
    cfg = OmegaConf.merge(cfg, {'mean_std_path': vqvae_train_cfg['mean_std_path']})
...
def run_training(
    ...
    # Setup.
    network_class = network.DFOTNetwork
    ...
    train_collate_fn = collate_dataclass_list(
        person_num=config.person_num, mean=mean, std=std, shuffle=True, is_mask_additional_person=config.is_mask_additional_person)
    val_collate_fn = collate_dataclass_list(
        person_num=config.person_num, mean=mean, std=std, shuffle=False, is_mask_additional_person=config.is_mask_additional_person)

    dataset_kwargs = {
        "mean_std_path": config.mean_std_path,
        "cache_files": True,
        "subseq_len": config.model_cfg.sequence_length,
        "slice_method": config.slice_method,
        "random_variable_len_min": config.random_variable_len_min,
        "random_variable_len_proportion": config.random_variable_len_proportion,
    }
